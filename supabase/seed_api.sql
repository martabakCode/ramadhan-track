-- ============================================================
-- SEED DATA FOR RAMADHAN APP (API VERSION)
-- Run this in Supabase SQL Editor AFTER running schema.sql and migrations
-- ============================================================
...
-- Note: Profiles are kept.

-- 2. Insert Challenges (3 types per day: Habit, Quran, Hadith)

-- TRACK 1: HABIT (General Good Deeds - adapted from user input)
...
-- TRACK 3: HADITH (Arbain Nawawi Daily)
INSERT INTO public.challenges (day_number, title, description, challenge_type, reward_points, challenge_date, icon, tips, payload)
SELECT 
    d, 
    'Hadits Arbain Ke-' || d, 
    'Baca dan renungkan makna Hadits Arbain Nawawi nomor ' || d || '.',
    'hadith',
    20,
    CURRENT_DATE + (d - 1),
    '📜',
    'Baca terjemahan dan ambil pelajaran utamanya.',
    jsonb_build_object('type', 'hadith', 'source', 'arbain-nawawi', 'number', d)
FROM generate_series(1, 42) as d -- Arbain has 42, but we stop at 30? Or cycle? Let's do 1-30.
WHERE d <= 30;
...
-- 4. Enable RLS (Should be already enabled but good to confirm)
